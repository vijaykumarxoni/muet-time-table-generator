   ...
   <body class="theme-blush">
										...
										<c:choose>
                             				<c:when test="${ (timetablewithoutclass.day.id == 1)  && (timetablewithoutclass.timeslot.id == loop.index) && (section_id == timetablewithoutclass.assignsubject.section.id )  && (batch_id == timetablewithoutclass.assignsubject.batch.id ) && (semester_id == timetablewithoutclass.assignsubject.semester.id ) }">
                             					<c:choose>
                             						...
                             						<c:otherwise>
                             							<td    class="${timetablewithoutclass.assignsubject.subject.code } assigned slots" id="assigned_${listoftimeslots.day.id}_${loop.index}_${timetablewithoutclass.assignsubject.id}"  >${timetablewithoutclass.assignsubject.subject.code} (${timetablewithoutclass.assignsubject.subject.type}) </td>
                             							<c:set var = "allocated"  value = "false"/>
                             						</c:otherwise>
                             					</c:choose>
			...
			$('.right td.drop').droppable({
				...
				onDrop:function(e,source){
					...
					if ($(source).hasClass('assigneding')){
						...
						var new_assign_id="assigned_"+day_id+"_"+slot_id+"_"+assignSubject_id;
	...
	<script>
		$(document).ready(function(){
			...
			var SPM_ARRAY = $(".SPM").map(function() { return this.id; });
			
			var assignsubject_id=$(".SPM").attr("assignsubject_id");
			
			var creditHour_id="credit-hour"+assignsubject_id;
			
			//alert(creditHour_id+"ok");
			
			
			var creditHour_value=$('#'+creditHour_id).text();
			
			creditHour_value=creditHour_value-SPM_ARRAY.length;
			$('#'+creditHour_id).text(creditHour_value);
			...
			if (creditHour_value=="0"){
				...
				$("#"+assignsubject_id).css({'opacity':'0.5', 'pointer-events': 'none'});
